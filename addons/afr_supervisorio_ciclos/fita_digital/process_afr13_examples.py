#!/usr/bin/env python3
"""Process all AFR13 example fitas, generate graphs, and profile performance."""
...
AFR13_EXEMPLOS_DIR = Path(__file__).parent / "reader_fita_digital" / "exemplos_fitas" / "afr13"


def process_afr13_example(fita_path):
    """Process single AFR13 example: read, generate graph, profile."""
    ...
    try:
        reader = ReaderFitaDigitalAfr13(str(fita_path))
        reader.read_file()
        reader.read_header()
        reader.read_body()
...
def main():
    """Process all AFR13 examples."""
    print("=" * 80)
    print("AFR13 EXAMPLE FITA PROCESSOR")
    print("=" * 80)

    if not AFR13_EXEMPLOS_DIR.exists():
        print(f"ERROR: Directory not found: {AFR13_EXEMPLOS_DIR}")
        return

    fita_files = sorted(AFR13_EXEMPLOS_DIR.glob("*.txt"))

    if not fita_files:
        print(f"No .txt files found in {AFR13_EXEMPLOS_DIR}")
        return
    ...
    for fita_path in fita_files:
        result = process_afr13_example(fita_path)
        if result:
